...
The Angular frontend uses standalone components, typed Reactive Forms, and a small Signals-based feature store. Angular Material provides accessible interactive controls, while Tailwind handles layout and status presentation. Understaffing and confidence remain independent: staffing is shown as `Short`, `Balanced`, or `Surplus`, while confidence is shown as a percentage and Low/Medium/High label. Text and icons accompany color.
